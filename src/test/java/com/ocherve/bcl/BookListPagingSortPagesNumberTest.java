 ...
 * @author herve_dev
 */
class BookListPagingSortPagesNumberTest {
	...
	void given_AscAttributeValue_When_gettingParametersOrCategory_Then_returnValidAttributeOrParameters() {
		...
		assertEquals(paging.getSortByPagesNumber(), "ASC");
		assertEquals(paging.toUrlParameters(), "sort_by_pages_number=ASC");
	}
	...
	void given_DescLowerAttributeValue_When_gettingParametersOrCategory_Then_returnValidAttributeOrParameters() {
		...
		assertEquals(paging.toUrlParameters(), "sort_by_pages_number=DESC");
	}
